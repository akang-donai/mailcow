...
test('summary shows uid, date, sender address and subject', () => {
  const line = formatSummary(42, {
...
test('summary labels a missing subject rather than printing undefined', () => {
  const line = formatSummary(7, { from: [{ address: 'a@b.tld' }] });
...
test('summary labels a missing sender rather than printing undefined', () => {
  const line = formatSummary(7, { subject: 'hi' });
